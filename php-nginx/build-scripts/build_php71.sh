...
curl -SL "https://downloads.php.net/~ab/php-$PHP71_VERSION.tar.gz" -o php71.tar.gz
# curl -SL "http://us2.php.net/get/php-$PHP71_VERSION.tar.gz.asc/from/this/mirror" -o php71.tar.gz.asc

##
# Skipping pgp checks, as there was no detailed view on who signs 7.1 code
##
...
mkdir -p ${PHP_SRC}
tar -zxf php71.tar.gz -C ${PHP_SRC} --strip-components=1
rm php71.tar.gz
# rm php71.tar.gz.asc
...
./configure \
    --prefix=${PHP71_DIR} \
    --with-config-file-scan-dir=${APP_DIR}:${PHP71_DIR}/lib/conf.d \
    --disable-cgi \
...
popd
rm -rf ${PHP_SRC}
strip ${PHP71_DIR}/bin/php ${PHP71_DIR}/sbin/php-fpm

# Create a directory for additional config files.
mkdir -p ${PHP71_DIR}/lib/conf.d

# Install shared extensions
${PHP71_DIR}/bin/pecl install mongodb
...
pushd /tmp/memcache
${PHP71_DIR}/bin/phpize
./configure --with-php-config=${PHP71_DIR}/bin/php-config
make
make install
popd
rm -rf /tmp/memcache
