...
if [ -z "$EXTENSIONS" ]; then
    ...
    EXTENSIONS="amqp,apcu,apcu_bc,libuv,cassandra-cpp-driver,cassandra,couchbase,ds,eio,ev,event,grpc,hprose,imagick,jsonc,jsond,krb5,libsodium,lua,lzf,mailparse,memcache,memcached,memprof,mongodb,oauth,phalcon,protobuf,raphf,pq,rdkafka,redis,seaslog,stackdriver_trace,stomp,suhosin,swoole,sync,tcpwrap,timezonedb,libv8,v8js,libvips,vips,xdebug,yaconf,yaf,yaml,zip"
fi
